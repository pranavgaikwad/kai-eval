...
describe("AnalysisTasksProvider Integration Tests", () => {
  let logger: Logger;
  ...
  it("should get tasks from analysis provider under different conditions", async () => {
    ...
    const totalInited = Object.values(providersSetup.providers).filter((p) =>
      p?.isInitialized(),
    ).length;
    ...
    expect(
      initialAnalysisTasks.some((task) => task.getUri() === beansXmlPath),
    ).toBe(true);
    const incidentsInBeansXmlBefore = initialAnalysisTasks.filter(
...
`;
    ...
    const catalogServiceJavaPath = path.join(
      coolstoreProjectPath,
      "src",
      "main",
      "java",
    ...
    expect(
      initialAnalysisTasks.some(
        (task) => task.getUri() === catalogServiceJavaPath,
      ),
    ).toBe(true);
    const incidentsInCatalogServiceJavaBefore = initialAnalysisTasks.filter(
      (task) =>
        task.getUri() === catalogServiceJavaPath &&
        task
          .toString()
          .includes("The package 'javax' has been replaced by 'jakarta'"),
    );
    const catalogServiceJavaOriginal = await fs.readFile(
      catalogServiceJavaPath,
      "utf-8",
    );
    ...
    expect(incidentsInBeansXmlAfter.length).toBeLessThan(
      incidentsInBeansXmlBefore.length,
    );
    const incidentsInCatalogServiceJavaAfter = newAnalysisTasks.filter(
      (task) =>
        task.getUri() === catalogServiceJavaPath &&
        task
          .toString()
    ...
    expect(incidentsInCatalogServiceJavaAfter.length).toBeLessThan(
      incidentsInCatalogServiceJavaBefore.length,
    );
    logger.info("Partial analysis triggered", {
      ...
      incidentsInCatalogServiceJavaBefore:
        incidentsInCatalogServiceJavaBefore.length,
      incidentsInCatalogServiceJavaAfter:
        incidentsInCatalogServiceJavaAfter.length,
    });
    ...
    expect(incidentsInBeansXmlAfterReset.length).toBe(
      incidentsInBeansXmlBefore.length,
    );
    const incidentsInCatalogServiceJavaAfterReset =
      analysisTasksAfterReset.filter(
        (task) =>
          task.getUri() === catalogServiceJavaPath &&
          task
            .toString()
    ...
    expect(incidentsInCatalogServiceJavaAfterReset.length).toBe(
      incidentsInCatalogServiceJavaBefore.length,
    );
