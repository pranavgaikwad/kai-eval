...
export class OptimizedEvaluationRunner implements EvaluationRunner {
  private readonly artifactsPath: string;
  private lastSnapshotId: number | undefined = undefined;

  constructor(
    private readonly config: KaiRunnerConfig,
    private readonly getTaskManager: GetTaskManagerFunction,
  ...
  private async setupApplicationGroup(
    ...
    // Setup task manager
    const { taskManager, shutdownFunc } = await this.getTaskManager(
      this.logger,
      testConfig,
    );
